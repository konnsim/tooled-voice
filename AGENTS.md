...
- `apps/api` contains the Hono API used locally with Node and deployed to Vercel. It owns JWT verification, OpenAI credential minting, local tool dispatch, encrypted integration credentials, OAuth refresh, and persistence. Linear capabilities come from the official hosted MCP attached during Realtime session creation.
