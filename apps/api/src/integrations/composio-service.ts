...
export class ComposioService {
  ...
  async connect(userId:string,toolkit:ComposioToolkit,callbackUrl:string,signal:AbortSignal):Promise<{authorizationUrl:string;connectionId:string}>{
    signal.throwIfAborted();const client=this.required();const session=await client.sessions.create(userId,{toolkits:[toolkit]});const connection=await session.authorize(toolkit,{callbackUrl,alias:`${toolkit}-${Date.now()}`});if(!connection.redirectUrl)throw new ApiError('INTEGRATION_UNAVAILABLE',`${toolkitName(toolkit)} did not return a connection link`,502,false);return{authorizationUrl:connection.redirectUrl,connectionId:connection.id};
  }
