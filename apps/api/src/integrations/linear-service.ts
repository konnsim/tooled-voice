...
import { integrationAccounts,integrationOauthStates,userProfiles } from '../database/schema.js';
...
export class LinearService {
  ...
  async createAuthorization(userId:string):Promise<{authorizationUrl:string}>{
    ...
    await this.database.delete(integrationOauthStates).where(and(eq(integrationOauthStates.userId,userId),eq(integrationOauthStates.provider,provider)));
    await this.database.insert(integrationOauthStates).values({userId,provider,stateHash:hashState(state),codeVerifier,redirectUri,expiresAt:new Date(Date.now()+stateLifetimeMs)});
    const url=new URL('https://linear.app/oauth/authorize');
    url.search=new URLSearchParams({client_id:config.clientId,redirect_uri:redirectUri,response_type:'code',scope:'read,write',state,code_challenge:codeChallenge,code_challenge_method:'S256'}).toString();
    return {authorizationUrl:url.toString()};
  }
  async completeAuthorization(code:string,state:string,signal:AbortSignal):Promise<void>{
    const [oauthState]=await this.database.update(integrationOauthStates).set({consumedAt:new Date(),updatedAt:new Date()}).where(and(eq(integrationOauthStates.provider,provider),eq(integrationOauthStates.stateHash,hashState(state)),isNull(integrationOauthStates.consumedAt),gt(integrationOauthStates.expiresAt,new Date()))).returning({userId:integrationOauthStates.userId,codeVerifier:integrationOauthStates.codeVerifier,redirectUri:integrationOauthStates.redirectUri});
    if(!oauthState)throw new ApiError('OAUTH_INVALID_STATE','The Linear authorization request is invalid or expired',400,false);
  ...
  private async validCredentials(userId:string,signal:AbortSignal,credentials:LinearCredentials):Promise<LinearCredentials>{
    ...
    const refresh=this.api.refreshToken(oauthConfig(),credentials.refreshToken,signal).then(token=>{
...
function oauthConfig():LinearOAuthConfig{
  ...
  if(!clientId||!clientSecret)throw new ApiError('INTEGRATION_UNAVAILABLE','Linear OAuth is not configured',503,false);
  return {clientId,clientSecret};
}
function linearRedirectUri():string{
  const value=process.env.LINEAR_REDIRECT_URI;
  if(!value)throw new ApiError('INTEGRATION_UNAVAILABLE','LINEAR_REDIRECT_URI is not configured',503,false);
  return value;
}
function hashState(state:string){return createHash('sha256').update(state).digest('hex')}
function toCredentials(token:LinearTokenResponse,fallbackRefreshToken?:string):LinearCredentials{
  return {accessToken:token.access_token,...(token.refresh_token||fallbackRefreshToken?{refreshToken:token.refresh_token??fallbackRefreshToken}:{}),tokenType:token.token_type,scope:token.scope.split(/[ ,]+/).filter(Boolean),...(token.expires_in?{expiresAt:new Date(Date.now()+token.expires_in*1000).toISOString()}: {})};
}
