...
describe('createRealtimeSession',()=>{
  ...
  it('attaches the official Linear MCP with the refreshed OAuth token',async()=>{vi.stubEnv('OPENAI_API_KEY','server-key');const fetcher=vi.fn().mockResolvedValue(new Response(JSON.stringify({value:'ephemeral',session:{id:'sess_2'}}),{status:200,headers:{'content-type':'application/json'}}));vi.stubGlobal('fetch',fetcher);await createRealtimeSession('user-1',AbortSignal.timeout(1000),'linear-access');const body=JSON.parse(String(fetcher.mock.calls[0]![1]?.body)) as {session:{tools:Array<Record<string,unknown>>}};expect(body.session.tools[1]).toEqual(expect.objectContaining({type:'mcp',server_label:'linear',server_url:'https://mcp.linear.app/mcp',authorization:'linear-access',require_approval:'always'}))});
});
