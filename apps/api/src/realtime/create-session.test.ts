...
describe('createRealtimeSession',()=>{
  ...
  it('creates a continuous gpt-realtime-2.1 session with semantic VAD and interruption',async()=>{
    ...
    const body=JSON.parse(String(init?.body)) as {session:{model:string;output_modalities:string[];audio:{input:{turn_detection:Record<string,unknown>}}}};
    expect(body.session).toMatchObject({model:'gpt-realtime-2.1',output_modalities:['audio']});
    expect(body.session.audio.input.turn_detection).toEqual({type:'semantic_vad',eagerness:'auto',create_response:true,interrupt_response:true});
  });
});
