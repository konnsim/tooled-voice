...
export async function createRealtimeSession(userId:string, signal:AbortSignal,linearAccessToken?:string) {
  ...
  const tools=[...realtimeTools,...(linearAccessToken?[{type:'mcp' as const,server_label:'linear',server_url:'https://mcp.linear.app/mcp',authorization:linearAccessToken,require_approval:'always' as const,server_description:'The user-connected Linear workspace. Use it for issues, projects, comments, cycles, initiatives, milestones, updates, labels, documents, and other Linear work.'}]:[])];
