...
export interface RealtimeMcpConnection {label:string;url:string;authorization?:string;approvalPolicy:'ask'|'automatic'}
export async function createRealtimeSession(userId:string, signal:AbortSignal,mcpConnection?:RealtimeMcpConnection) {
  ...
  const tools=[...realtimeTools,...(mcpConnection?[{type:'mcp' as const,server_label:mcpConnection.label,server_url:mcpConnection.url,...(mcpConnection.authorization?{authorization:mcpConnection.authorization}:{}),require_approval:mcpConnection.approvalPolicy==='automatic'?'never' as const:'always' as const,server_description:'The user-connected tool services. Discover and use the appropriate connected service for the request.'}]:[])];
