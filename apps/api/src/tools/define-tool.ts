...
import type { IntegrationServices } from '../integrations/types.js';
...
export interface ToolExecutionContext { user: AuthenticatedUser; requestId: string; database: Database; integrations: IntegrationServices; logger: Logger; signal: AbortSignal }
...
export interface RegisteredToolDefinition { name:string; description:string; input:z.ZodType; output?:z.ZodType; permissions:readonly string[]; retry:{enabled:boolean}; execute(input:unknown,context:ToolExecutionContext):Promise<unknown> }
