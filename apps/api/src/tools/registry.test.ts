...
describe('tool registry',()=>{
  it('registers tools without a switch and exposes JSON schema',()=>{expect(toolRegistry.get('getCurrentTime')).toBe(getCurrentTime);expect(toolRegistry.has('createLinearIssue')).toBe(true);expect(realtimeTools).toHaveLength(2);expect(realtimeTools[0]?.parameters).toMatchObject({type:'object'})});
