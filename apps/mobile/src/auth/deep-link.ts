...
export async function handleAuthDeepLink(url:string):Promise<void>{
  ...
  if(callback.protocol!=='tooledvoice:'||callback.hostname!=='auth'||callback.pathname!=='/callback')return;
...
export function subscribeToAuthDeepLinks(onError:(message:string)=>void):()=>void{
  const open=(url:string)=>{void handleAuthDeepLink(url).catch(error=>onError(error instanceof Error?error.message:'Could not confirm your account.'))};
  void Linking.getInitialURL().then(url=>{if(url)open(url)}).catch(error=>onError(error instanceof Error?error.message:'Could not open the confirmation link.'));
