...
type Listener=(event:{state?:ConnectionState;muted?:boolean;transcript?:{id:string;role:'user'|'assistant';text:string;final:boolean};error?:string})=>void;
...
export class RealtimeClient {
  private pc:RTCPeerConnection|undefined; private channel:Channel|undefined; private stream:MediaStream|undefined; private microphone:MediaStreamTrack|undefined; private remote:MediaStream|undefined; private conversationId:string|undefined; private closed=false; private retries=0; private responseActive=false; private userSpeaking=false; private desiredMuted=false; private reconnecting=false; private openTimer:ReturnType<typeof setTimeout>|undefined;private readonly streamingTranscripts=new Map<string,string>();
  ...
  async connect(reconnecting=false):Promise<void>{
    ...
    try {
      ...
      channel.addEventListener('message',event=>void this.onEvent(event.data)); channel.addEventListener('open',()=>{if(this.openTimer)clearTimeout(this.openTimer);this.openTimer=undefined;this.retries=0;microphone.enabled=!this.desiredMuted;this.listener({muted:this.desiredMuted});this.state('connected')}); channel.addEventListener('error',()=>{if(this.channel===channel)void this.reconnect()});channel.addEventListener('close',()=>{if(this.channel===channel)void this.reconnect()});
  ...
  setMuted(muted:boolean){this.desiredMuted=muted;if(this.microphone)this.microphone.enabled=!muted;this.listener({muted})}
  ...
  private async onEvent(raw:unknown){if(typeof raw!=='string')return;let event:any;try{event=JSON.parse(raw)}catch{return}if(!event||typeof event.type!=='string')return;switch(event.type){case'input_audio_buffer.speech_started':this.userSpeaking=true;this.state('listening');break;case'input_audio_buffer.speech_stopped':this.userSpeaking=false;this.state('thinking');break;case'conversation.item.input_audio_transcription.delta':this.transcriptDelta(event,'user');break;case'conversation.item.input_audio_transcription.completed':this.transcriptDone(event,'user');break;case'conversation.item.input_audio_transcription.failed':this.listener({error:typeof event.error?.code==='string'?event.error.code:'TRANSCRIPTION_FAILED'});break;case'response.created':this.responseActive=true;if(!this.userSpeaking)this.state('thinking');break;case'response.audio.delta':case'response.output_audio.delta':if(!this.userSpeaking)this.state('speaking');break;case'response.output_audio_transcript.delta':this.transcriptDelta(event,'assistant');break;case'response.audio_transcript.done':case'response.output_audio_transcript.done':this.transcriptDone(event,'assistant');break;case'response.done':this.responseActive=false;this.state(this.userSpeaking?'listening':'connected');break;case'response.function_call_arguments.done':await this.handleTool(event);break;case'error':this.listener({error:typeof event.error?.code==='string'?event.error.code:'REALTIME_ERROR'});break}}
  ...
  private transcriptDone(event:any,role:'user'|'assistant'){if(typeof event.item_id!=='string'||typeof event.transcript!=='string')return;const transcript=event.transcript.trim();this.streamingTranscripts.delete(event.item_id);this.listener({transcript:{id:event.item_id,role,text:transcript,final:true}});if(transcript)this.persist({role,kind:'transcript',transcript})}
  ...
  disconnect(complete=false){this.closed=true;this.cleanup(true);if(complete){this.desiredMuted=false;this.listener({muted:false})}if(complete&&this.conversationId){void finishConversation(this.conversationId,'completed').catch(()=>this.listener({error:'HISTORY_PERSIST_FAILED'}));this.conversationId=undefined}this.state('disconnected')}
